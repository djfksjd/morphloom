...
const output = {
  ...
  capabilityMatrix: [
    ...
    { capability: 'named editable facial controls with spatially localized deltas preserved through GLB', img2threejs: 'not established in pinned audit', morphloom: domainProof.animation?.pass && domainProof.animation?.metrics?.facialMorphLocalizedTargets === 5 ? '5/5 non-zero targets with >=98% head-region localization' : 'blocked' },
