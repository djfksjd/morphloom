...
| OBJ/STL/PLY 실제 정적 메시 납품 | 고정 커밋 감사에서 확인되지 않음 | 구현·과거 Blender 통과; 이전 ZIP이 compiler revision을 기록하지 않아 현재 0.25.0 증거는 재캡처 전 차단 |
| USDZ 규격 검사 | 고정 커밋 감사에서 확인되지 않음 | 과거 Apple usdchecker 통과; 현재 revision-bound 에셋 팩 재생성 필요 |
| 준실무 납품성 | Three.js 전시 강점 | IR/GLB/OBJ/STL/PLY/USDZ/Figma SVG 구현; 현재 GLB Blender 5분야 통과, 브라우저·정적 팩·Unity 증거는 차단 상태 |
...
- 브라우저 GLB 검증은 동일 입력을 하나의 작업으로 합치고 서로 다른 무거운 작업을 직렬화합니다. 대기 작업은 4개로 제한하며, 오래된 결과가 현재 선택을 덮지 못하고 현재 항목을 다시 선택해도 이미 통과한 증거를 지우지 않습니다. compiler 0.23.0의 실제 Chromium 세션에서 9개 대표 자산을 순차 재검증했고 오류·경고 0개였습니다. 캐릭터 2종은 모프 10/10의 정점 수·변형 길이 합·제곱합·최대값까지 왕복 비교했습니다.
...
| 정적 메시·Apple AR 실제 납품 검사 | 고정 커밋 감사에서 확인되지 않음 | 과거 동일 건축 장면은 Blender·usdchecker 통과; 현재는 낡은 ZIP의 compiler revision 누락을 탐지해 재캡처 전 차단 |
...
| 건축 실측 치수 재검증 | 고정 커밋 감사에서 확인되지 않음 | 지원—실측·데이터시트 근거만 전체/부재 X·Y·Z 크기·최소·최대·중심으로 잠그고 완성 메시 월드 좌표를 재측정; 수평 외곽이 같아도 2700→2400 mm 높이 회귀 차단; GLB 감사 지문 보존 |
| 부품 기준점 피치 재검증 | 고정 커밋 감사에서 확인되지 않음 | 지원—실제 부품 경계 안의 로컬 기준점을 월드 좌표로 변환해 홀·핀·렌즈·커넥터 중심의 축별/3차원 피치를 재측정; 25.0→24.5 mm 이동 차단; 실제 GLB 재열기에서 감사 지문 보존 |
...
| 자동 테스트 폭 | 코어 1,083개 실행 확인 | 258개—개수보다 납품 계약 회귀, 선형광·해상도 정규화 표면 복원과 잡음 억제, 낡은 에셋 팩의 새 컴파일러 증거 위장 차단, 실제 질감 픽셀/의미 샘플러와 PBR 스칼라·광학 재질 왕복, 중복 식별자 뒤의 비호환 재질 은폐 차단, 중립 렌더의 카메라·전체 캔버스 위치/크기·픽셀비·색/톤/조명 프로토콜·같은 GLB·실제 파일/영수증 해시 위조 차단에 집중 |
...
결과는 `benchmarks/competitive-latest.json`, `benchmarks/blender-cross-domain-latest.json`, `benchmarks/unity-cross-domain-latest.json`, `benchmarks/static-delivery-latest.json`에 저장됩니다. 현재 5분야 Blender 증명은 실제 상호운용성과 납품 바이트 무결성을 확인하지만 동일 입력의 시각적 우승을 뜻하지 않습니다. 정적 포맷 보고서는 이전 에셋 팩의 compiler revision 누락을 발견해 재캡처 전까지 차단합니다. Unity 보고서는 라이선스나 에디터 실패를 최대 두 번의 제한된 복구 후 구조화해 기록하며 임포트하지 못한 실행을 통과로 바꾸지 않습니다. 실제 시각 품질의 우열은 동일 입력 지문, 동일 목표, 동일 보정 카메라, 실제 WebGL 캡처, 중요 특징 영역, 최소 5명의 중복 없는 균형 블라인드 평가가 있어야 확정됩니다. 이 증거가 없으면 자동 지표가 앞서도 보고서는 `claimAllowed: false`를 유지합니다.
