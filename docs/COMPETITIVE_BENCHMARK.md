...
추가로 Morphloom은 분야마다 다른 납품 계약을 실행합니다. 건축은 검증 평면·폐쇄 셸·80% 이상 미세표면, 애니메이션은 실제 스켈레톤·정규화 웨이트·클립 보존, 게임은 10만 tris 예산·UV/노멀·토폴로지, 3D 프린팅은 mm·폐쇄 체적·0.8 mm 이상 선언 형상을 각각 차단 조건으로 사용합니다. 현재 8개 잠금 사례의 기술·판정·납품·거절 안전성은 모두 100%입니다.
...
| 실제 스켈레톤·스킨·애니메이션 GLB | 고정 커밋 감사에서 확인되지 않음 | 지원—17 bones, 1 clip/2 tracks 재열기 |
| 게임 예산·UV·노멀·토폴로지 게이트 | 고정 커밋 감사에서 확인되지 않음 | 지원 |
| 3D 프린팅 mm·폐쇄 체적·최소 형상 게이트 | 고정 커밋 감사에서 확인되지 않음 | 지원 |
| 자동 테스트 폭 | 1,083개 실행 확인 | 110개—개수보다 납품 계약 회귀에 집중 |
