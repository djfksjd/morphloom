...
export function evaluateQuality(
  ...
  const deliveredRig = Boolean(deliveryAudit?.status === 'pass'
    ...
    && (deliveryAudit.source?.morphTargets ?? 0) >= 5);
...
export function evaluateProductQuality(
  ...
  const surfaceScore = surfaces
    ? isSurfaceBenchmark
      ...
      && surfaces.unobservedSynthesizedMaterials >= surfaces.referenceProjectedMaterials
      ? 100
  ...
  const checks: QualityCheck[] = [
    ...
    },
    {
      ...
      detail: surfaces
        ? `${surfaces.distinctFinishes}종 finish · micro-normal ${surfaces.microNormalMaterials}/${surfaces.authoredMaterials} · roughness-map ${surfaces.roughnessMappedMaterials}/${surfaces.authoredMaterials} · 참조 투영 ${surfaces.referenceProjectedMaterials}개 · 선형 de-light ${surfaces.referenceDelightedMaterials}/${surfaces.referenceProjectedMaterials} · 사진 파생 normal+roughness ${surfaces.referenceReliefMaterials}개 · 숨은면 재질합성 ${surfaces.unobservedSynthesizedMaterials}개${surfaces.referenceProjectionFingerprints.length ? ` · 입력 fingerprint 검증 · 불규칙성 ${surfaces.maximumReferenceIrregularity.toFixed(2)}` : ''} · 이방성 ${surfaces.anisotropicMaterials}${detailAudit ? ` · IR 표면 ${Math.round(detailAudit.explicitSurfaceCoverage * 100)}%` : ''}`
