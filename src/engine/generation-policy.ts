...
export function expandGenerationBrief(
  ...
  return {
    ...
    agentPrompt: [
      ...
      `Render and inspect these review modes as needed: ${REVIEW_MODES.join(', ')}. Calibrate every photographic source camera from at least four stable anchors, compare that locked view first, and produce foreground-normalized top/middle/bottom interior evidence rather than judging an attractive free camera. Compare each material region for base colour, luminance, microstructure, and directional response. When at least two compatible orthographic silhouettes exist, carve a bounded welded visual hull before detail work; report unconstrained axes and never invent silhouette-invisible concavities. Reproject the result into every supplied silhouette, record per-view IoU and false-positive/negative fractions, and block delivery below 0.75 minimum-view or 0.85 confidence-weighted IoU even when the mesh is watertight. For evidence-supported organic blends and recesses, use a bounded editable implicitSurface graph rather than overlapping primitive shells; require padded bounds, triangle budget, and manifold Surface Nets output after no more than four deterministic resolution refinements. Use diagnostic views to expose form, surface, topology, attachments, and hidden relationships.`,
