...
![Tests](https://img.shields.io/badge/tests-481%20passing-28a879?style=flat-square)
...
The external ABO pilot covers three cases, 18 assets, and two multi-view scale inferences. The semantic lamp passes its ground-truth GLB audit. The fan case delivers 101 editable parts plus deterministic Khronos-valid GLB reopen, but its four-view silhouette and ground-truth shape gates still block release. After separating a rigid capture-pose residual, its minimum whole-silhouette, primary-mass, and thin-feature scores are 0.597, 0.674, and 0.710; the 4,096-point shape audit reports 4.15% maximum dimension error, 0.0551 RMS Chamfer, 0.1151 P95 distance, and 54.8% coverage. Locked-coordinate missing/excess localization reports 0.743 minimum recall and 0.562 minimum precision. Even when the GT is one unnamed merged mesh, nearest-surface attribution assigns missing-shape responsibility back to editable candidate parts and emits bounded trials only with an explicit metre-to-millimetre conversion and a 100 mm ceiling. A fingerprint covering the point clouds, alignment, and thresholds must match the recovery action, so stale attribution cannot drive a later edit. In this pilot a 75% motor-shell translation improved RMS, P95, and coverage while keeping every protected regression within 0.0002. The separate semantic-ablation/screen-residual route still withholds an automatic 3D partial edit unless two distinct orthographic views have verified absolute pose, intrinsics, and pixel scale. The selected capture residual reaches 80° and is not independently measured, so it remains a diagnostic rather than camera or delivery evidence. The fan remains `productionReady: false`, and global superiority is not claimed.
