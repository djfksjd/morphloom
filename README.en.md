...
The external ABO pilot covers three cases, 18 assets, and two multi-view scale inferences. The semantic lamp passes its ground-truth GLB audit. The fan case delivers 101 editable parts plus deterministic Khronos-valid GLB reopen, but its four-view silhouette and ground-truth shape gates still block release. After separating a rigid capture-pose residual, its minimum whole-silhouette, primary-mass, and thin-feature scores are 0.597, 0.677, and 0.720; the 4,096-point shape audit reports 4.15% maximum dimension error, 0.0546 RMS Chamfer, 0.1151 P95 distance, and 55.5% coverage. Locked-coordinate missing/excess localization reports 0.747 minimum recall and 0.564 minimum precision. Even when the GT is one unnamed merged mesh, nearest-surface attribution assigns missing-shape responsibility back to editable candidate parts and emits bounded trials only with an explicit metre-to-millimetre conversion and a 100 mm ceiling. Per-part robust 10–90% surface spans also produce a single-axis scale candidate mapped back through rotated local axes, with change capped at 20%. Open tubes, pipes, and wires compare whole-part translation with endpoint/control-point deformation; extruded outlines, lathe profiles, and blade-loft sections can likewise move one evidence-facing existing control without changing point or topology inventory. Tubes pair excess and missing samples around each control anchor so local direction does not default to the component centroid. A fingerprint covering the point clouds, alignment, thresholds, and controls must match the recovery action, so stale attribution cannot drive a later edit. If one candidate fails during mesh application or evaluation, recovery records the stage and reason, then continues from the immutable checkpoint. Three-round recovery re-attributes the current geometry on every checkpoint under a 128-trial total budget and an original-checkpoint cumulative regression ceiling. The latest run compared 92 candidates and selected 75% motor-shell and right-stand translations in sequence. Five scale candidates qualified as individual improvements but ranked below the final translation combination; the last four evidence-attributed tube-control candidates regressed protected gates and were all rolled back automatically. The separate semantic-ablation/screen-residual route still withholds an automatic 3D partial edit unless two distinct orthographic views have verified absolute pose, intrinsics, and pixel scale. The selected capture residual reaches 80° and is not independently measured, so it remains a diagnostic rather than camera or delivery evidence. The fan remains `productionReady: false`, and global superiority is not claimed.
