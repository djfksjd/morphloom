...
3. Identify returns, wings, and entrance projections before laying out rooms. A single bounding rectangle is forbidden when the source contains voids or projections.
...
Record `planFootprintVerified: true` only after comparing the source plan and a plan-view render. Record known void and projection counts in metadata. Add a regression test for each signature form feature, such as a named entrance slab or the expected number of courtyard voids.
