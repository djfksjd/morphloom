...
For a cross-engine winner claim, require the domain minimum number of distinct calibrated views, identical reference bytes and feature rectangles for both candidates, unique render and scene fingerprints per view, non-empty foreground in every critical region, and a balanced blind panel of at least five unique raters. Refuse reused captures, candidate-identical render pixels, per-candidate easy regions, or an average that hides one failed view. A one-view automatic lead is regression evidence only and must remain `claimAllowed: false`.
