...
- Require a real `SkinnedMesh`, named bone hierarchy, normalized weights with no more than four non-zero influences per vertex, and deformation-safe closed body topology.
...
- Report LODs, collision, texture resolution, animation set, and target engine import settings as missing when they were not produced; do not infer them from a successful GLB load.
...
- Check boundary, non-manifold and degenerate counts before export, then reopen the exported mesh. Keep printer-specific overhang, orientation, supports, shrinkage, tolerances, and material process as downstream warnings until a target printer/slicer profile is provided.
