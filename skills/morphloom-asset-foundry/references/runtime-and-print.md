...
- Require actual lower-detail render geometry for every declared LOD and preserve its skin attributes when animated. For the bundled runtime base, LOD1 must retain 10–85% of LOD0 triangles, 100% valid normalized skin weights and bone indices, no more than 3% neutral-pose bounds error, 1.2% neutral silhouette-envelope error, 4% posed bounds error, and 1.6% posed silhouette-envelope error. Measure envelopes across all three axes in 32 slices after applying a real driven-joint perturbation; part count or matching global bounds alone cannot prove LOD quality. Every collision primitive needs a unique id, finite positive dimensions, an existing bone binding, intersection with the rendered body bounds, and adequate combined vertical coverage; preserve the exact collision-manifest fingerprint through GLB reopening. Report LODs, collision, texture resolution, animation set, and target engine import settings as missing when they were not produced; do not infer them from a successful GLB load or metadata counts alone.
