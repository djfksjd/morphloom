...
4. Convert structure into semantic edit units. Preserve holes, gaps, courtyards, seams, openings, connectors, and other negative space as geometry—not texture or prose. Use measured or manufacturer dimensions where available and mark unsupported depth or hidden construction `estimated` or `inferred`. When at least two compatible orthographic silhouettes exist, use the bounded `visualHull` AssemblyIR operation as a form constraint before detail work; keep its unconstrained axes and concavity limitation in the evidence record. Reproject the carved volume into every supplied mask and require both minimum-view and confidence-weighted silhouette agreement; a watertight mesh with poor source-view agreement remains a blocked review draft.
...
8. Deliver editable GLB plus source IR, comparison renders, and an evidence-aware audit. Reopen the exact GLB bytes and apply the target domain contract; a generic mesh score cannot substitute for skeleton/weight checks, a game budget, or print units/features. State only the remaining survey, multi-view, bench-test, rigging, slicing, or artist work that materially affects the requested use.
