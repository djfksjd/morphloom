...
`quality:production`은 더 엄격합니다. 분야마다 서로 다른 동일 입력 사례 3개, 반복 생성, 브라우저와 Blender 재열기, 게임 엔진/슬라이서 재열기, img2threejs와의 자동 점수 우위, 최소 5명의 순서 균형 블라인드 우위를 모두 요구합니다. 입력을 먼저 잠그고 양쪽 후보를 봉인한 뒤에만 정답을 공개해야 하며, 개발 노출 원장에 기록된 튜닝·디버깅 사례는 홀드아웃으로 셀 수 없습니다. 한 항목이라도 없으면 전 분야 우세나 실무 즉시 납품을 선언하지 않습니다.

현재 외부 ABO 파일럿은 3개 사례·18개 자산과 2개 다중 시점 크기 추론을 검증합니다. 의미 기반 램프는 정답 GLB 감사까지 통과했습니다. 팬 사례는 101개 편집 부품, 결정론적 GLB, Khronos/독립 재열기를 통과했지만 네 시점 윤곽과 정답 형상은 아직 차단됩니다. 강체 촬영 포즈 잔차를 분리한 뒤 최저 전체 윤곽·주요 질량·얇은 부재 점수는 0.597·0.677·0.720이며, 4,096점 형상 감사는 최대 치수 오차 4.15%, RMS Chamfer 0.0546, P95 0.1151, 커버리지 55.5%입니다. 잠긴 화면 좌표에서 누락·과잉 형상을 따로 추적한 최저 회수율·정밀도는 0.747·0.564입니다. 정답 GLB가 이름 없는 단일 메시여도 최근접 표면으로 누락 책임을 편집 부품에 귀속하고, 명시적 m↔mm 변환과 100 mm 상한 안에서 이동 후보를 만듭니다. 부품별 10–90% 강건 표면 범위는 회전된 로컬 축으로 되돌린 단일 축 크기 후보도 만들며 최대 변화량은 20%로 제한됩니다. 열린 튜브·배관·전선은 전체 이동과 별도로 끝단/제어점 국소 변형을 시험하며, 압출 외곽·회전체 프로파일·칼날 로프트도 기존 점과 토폴로지 수를 유지한 채 근거 방향의 한 점만 수정할 수 있습니다. 튜브는 제어점 주변의 과잉·누락 표면을 따로 짝지어 부품 평균이 아닌 국소 이동 방향을 계산합니다. 점군·정렬·문턱·제어점을 포함한 귀속 근거 지문이 복구 액션과 일치하지 않으면 오래된 수정을 실행하지 않습니다. 후보 하나의 메시 적용이나 평가가 실패해도 원본 체크포인트에서 다음 후보를 계속 시험하고 실패 단계·사유를 영수증에 남깁니다. 3라운드 복구는 매번 현재 형상을 다시 귀속하며 총 128후보 상한과 최초 체크포인트 기준 누적 회귀 한도를 지킵니다. 최신 실행은 92후보를 비교해 모터 셸과 오른쪽 스탠드의 75% 이동을 선택했습니다. 축척 후보 5개는 개별 개선이었지만 최종 이동 조합보다 낮았고, 마지막 근거 귀속 튜브 제어점 4후보는 보호 지표를 회귀시켜 모두 자동 폐기됐습니다. 부품 제거 영향과 화면 잔차를 연결하는 별도 경로는 서로 다른 두 직교 시점의 절대 자세·렌즈·픽셀 축척이 검증되지 않으면 자동 3D 부분수정을 허용하지 않습니다. 선택 촬영 잔차가 최대 80°이고 독립 측정되지 않았으므로 그 보정은 진단에만 쓰며 카메라 정확도나 납품 근거로 인정하지 않습니다. 따라서 팬은 `productionReady: false`이며 전 분야 우세도 아직 주장하지 않습니다.
