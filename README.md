...
현재 외부 ABO 파일럿은 3개 사례·18개 자산과 2개 다중 시점 크기 추론을 검증합니다. 의미 기반 램프는 정답 GLB 감사까지 통과했습니다. 팬 사례는 101개 편집 부품, 결정론적 GLB, Khronos/독립 재열기를 통과했지만 네 시점 윤곽과 정답 형상은 아직 차단됩니다. 강체 촬영 포즈 잔차를 분리한 뒤 최저 전체 윤곽·주요 질량·얇은 부재 점수는 0.597·0.674·0.710이며, 4,096점 형상 감사는 최대 치수 오차 4.15%, RMS Chamfer 0.0551, P95 0.1151, 커버리지 54.8%입니다. 잠긴 화면 좌표에서 누락·과잉 형상을 따로 추적한 최저 회수율·정밀도는 0.743·0.562입니다. 정답 GLB가 이름 없는 단일 메시여도 최근접 표면으로 누락 책임을 편집 부품에 귀속하고, 명시적 m↔mm 변환과 100 mm 상한 안에서 이동 후보를 만듭니다. 점군·정렬·문턱을 포함한 귀속 근거 지문이 복구 액션과 일치하지 않으면 오래된 수정을 실행하지 않습니다. 이 파일럿에서는 모터 셸의 75% 이동이 RMS·P95·커버리지를 모두 개선했고 다른 보호 지표의 회귀는 0.0002 이내였습니다. 부품 제거 영향과 화면 잔차를 연결하는 별도 경로는 서로 다른 두 직교 시점의 절대 자세·렌즈·픽셀 축척이 검증되지 않으면 자동 3D 부분수정을 허용하지 않습니다. 선택 촬영 잔차가 최대 80°이고 독립 측정되지 않았으므로 그 보정은 진단에만 쓰며 카메라 정확도나 납품 근거로 인정하지 않습니다. 따라서 팬은 `productionReady: false`이며 전 분야 우세도 아직 주장하지 않습니다.
