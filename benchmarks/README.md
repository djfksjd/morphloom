...
- animation requires closed skinned LOD meshes, at least 45 real bones, 30 named finger bones with actually weighted hand vertices, at least ten finger tracks, normalized ≤4-influence weights, measured non-zero joint deformation, UVs, all five semantic idle/walk/run/turn/gesture clips with at least 50 tracks, and exact GLB clip/binding preservation;
- game requires the delivered triangle budget, no non-manifold or degenerate geometry, UVs, normals, a real skeleton, all five semantic runtime clips, an actual skinned LOD1, collision primitives, and PBR surfaces;
